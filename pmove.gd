...
func fly_move():
	...
	if wishdir != Vector3.ZERO:
		velocity = velocity.linear_interpolate(wishdir * movespeed, ACCELERATE * deltaTime) 
	else:
...
func ground_move():
	...
	for _i in range(ccd_max):
		...
		var collision = move_and_collide(ccd_step * deltaTime, false)
		if collision:
			var col_normal = collision.get_normal()
			var col = collision.get_collider()
			if col.is_in_group("bodies"):
				col.apply_central_impulse(-col_normal * velocity.length() * R_PUSH)
				velocity = velocity.slide(col_normal)
			else:
				if col_normal[1] < 0.7 and !is_dead:
					var stepped = step_move(global_transform.origin, velocity.normalized() * 10)
					if !stepped and velocity.dot(col_normal) < 0:
						velocity = velocity.slide(col_normal)
				else:
					velocity = velocity.slide(col_normal)
